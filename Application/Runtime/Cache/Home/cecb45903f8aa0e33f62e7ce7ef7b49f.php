...
<style>
    ...
    .newslist dl dd{height:34px;line-height: 34px;color: #666;background-color: #f8f7f7;overflow: hidden;}
</style>
<script>
$(function(){
    var domain = document.domain;
    $('.godetail').click(function(){
        var newsid=$(this).closest('dl').find('.newsid').val();
        window.open("http://"+domain+"/Index/newscontain?id="+newsid);
    })
})
</script>
<div class="banner">
    <div class="banner-contain w1200 pr">
        ...
        <div class="bc-right tl pa" style="left:200px;top:20px;">
            ...
            <p class="mt20 wow fadeInUp">定时更新互联网最新动态</p>
        </div>
    </div>
</div>
<div class="news">
    <div class="newslist w1200">
        <?php if($newslist[0] == ''): ?><p style="text-align: center;font-size: 20px;color:#888;">暂无动态</p><?php endif; ?>
        <?php if(is_array($newslist)): $i = 0; $__LIST__ = $newslist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><dl>
            <input class="newsid" type="hidden" value="<?php echo ($vo["id"]); ?>">
            <dt><a href="javascript:;" class="godetail" target="_blank"><?php echo ($vo["title"]); ?></a> <span class="time"  style="font-size: 16px;">20<?php echo (date("y-m-d",$vo["data"])); ?></span></dt>
